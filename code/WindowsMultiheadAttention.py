...
class WindowsMultiheadAttention(nn.Module):# 基于窗口的多头注意力
    ...
    def forward(self, x):
        ...
        x = F.unfold(x,kernel_size=self.window_size,stride=self.window_size)# [bs, windows_depth, nums_window]
        
        x=x.reshape(bs,windows_depth)
        
        bs,windows_depth,nums_window=x.shape
        
        x=x.transpose(1,2)# [bs, nums_window, windows_depth]
